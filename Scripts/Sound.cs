...
public class Sound {
	...
	public static readonly string[] seKeyName = {
		"Jump", "Crush", "Frag", "Reverse", "Force", "Fast", "Slow", "Rejump", "ColorBlock", "Goal",
	};
	...
	bool _PlayBgm(string key) {
		...
		// 再生
		AudioSource source = _GetAudioSource(eType.Bgm);
		source.loop = true;
	...
	bool _PlaySe(string key, int channel=-1) {
		...
		{
			// チャンネル指定
			AudioSource source = _GetAudioSource(eType.Se, channel);
			source.clip = _data.Clip;
			source.Play();
		}
		else
		{
			// デフォルトで再生
			AudioSource source = _GetAudioSource(eType.Se);
